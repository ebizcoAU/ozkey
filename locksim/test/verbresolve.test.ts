...
test("unlock resolves to the real DP on a real profile, the fiction only on the fiction", () => {
  ...
  const real = resolveVerbDown(getProfile("tuya-luona-ds013-t3"), "lock.unlock");
  ...
  // ozkie-legacy-v0 was deleted 2026-08-20. DP 1 must now resolve NOWHERE as an
  // unlock: it was ours, and no real product implements it.
  for (const id of PROFILE_IDS) {
    const got = resolveVerbDown(getProfile(id), "lock.unlock");
    assert.notEqual(got?.dp, 1, `${id}: DP 1 is our fiction and must not unlock`);
  }
});
...
test("DP 10 is reachable only when named, never as the default", () => {
  ...
  const p = getProfile("tuya-luona-ds013-t3");
