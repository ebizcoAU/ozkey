...
void startBle() {
  ...
  // ── WHICH BRIDGE IS THIS? (operator, 2026-08-12) ────────────────────────
  //
  // Same defect the locks had: every bridge advertises the identical name
  // "OZBRIDGE", so an app scanning two of them shows two indistinguishable
  // rows and can only resolve identity AFTER connecting and reading
  // info.device_id. Mirrors doorlock-1.55's fix so ftpos parses ONE shape.
  //
  // ⚠ NOT the CoreBluetooth UUID the phone displays — that is generated by
  // the phone, differs per handset, and never crosses the air. The identifier
  // has to be one this device owns and broadcasts.
  //
  // 4 bytes of MAC, rendering as the same 8 hex characters that lead this
  // bridge's device_id: `ozb-98a316a7e638` broadcasts 98 A3 16 A7, so the app
  // shows `98a316a7`, a clean prefix of what the panel already prints in full.
  //
  // BUDGET — differs from the lock's, so this was checked rather than copied.
  // The ADV is full at 31/31 (flags 3 + 128-bit UUID 18 + name "OZBRIDGE" 10),
  // which is precisely why the name stays THERE and this goes in the scan
  // response: service data with the 128-bit UUID costs 2 + 16 + 4 = 22 of the
  // scan response's own 31, leaving the advertised name untouched. Putting it
  // in the ADV instead would have displaced the service UUID that BANOI's
  // Android `withServices` filter keys off — the mistake made once on the
  // lock side and not repeated here.
  {
    // Local nibble decoder — the lock's ozdoorlock_core.h has one, but this
    // sketch does not include it (bridge and lock share only oztime.h).
    auto nib = [](char c) -> int {
      if (c >= '0' && c <= '9') return c - '0';
      if (c >= 'a' && c <= 'f') return c - 'a' + 10;
      if (c >= 'A' && c <= 'F') return c - 'A' + 10;
    ...
    for (int i = 0; i < 4 && dash >= 0; i++) {
      const int hi = nib(deviceId.charAt(dash + 1 + i * 2));
      const int lo = nib(deviceId.charAt(dash + 2 + i * 2));
      if (hi < 0 || lo < 0) break;
      sd += (char)((hi << 4) | lo);
    }
    if (sd.length() == 4) {
      ...
      Serial.printf("[BLE] broadcast id = %.8s (scan response)\n",
                    deviceId.c_str() + dash + 1);
    }
  }
